# Run gptp with Automotive profile as Administrator (Stable Mode - No Enhanced Debugging)
param()
...
Write-Host "Starting gptp with Automotive profile (STABLE MODE), MAC: $mac" -ForegroundColor Green

# Generate timestamped log filename with full paths
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logFile = Join-Path $scriptDir "build\Debug\gptp_automotive_stable_$timestamp.log"
$errorFile = Join-Path $scriptDir "build\Debug\gptp_automotive_stable_$timestamp.err"
...
try {
    Write-Host "Executing: $exePath -Automotive $mac (NO debug-packets)" -ForegroundColor Cyan
    ...
    & $exePath -Automotive $mac 2>&1 | Tee-Object -FilePath $logFile
