# Run gptp with Automotive profile as Administrator
param()
...
Write-Host "Starting gptp with Automotive profile, MAC: $mac" -ForegroundColor Green

# Generate timestamped log filename with full paths
$timestamp = Get-Date -Format "yyyyMMdd_HHmmss"
$logFile = Join-Path $scriptDir "build\Debug\gptp_automotive_$timestamp.log"
$errorFile = Join-Path $scriptDir "build\Debug\gptp_automotive_$timestamp.err"
...
try {
    Write-Host "Executing: $exePath -Automotive $mac -debug-packets" -ForegroundColor Cyan
    Write-Host "Log file: $logFile" -ForegroundColor Cyan
    
    # Run gptp and capture all output
    & $exePath -Automotive $mac -debug-packets 2>&1 | Tee-Object -FilePath $logFile
